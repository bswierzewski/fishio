...
public class AgeRange : ValueObject
{
    ...
    public AgeRange(int? minAge = null, int? maxAge = null)
    {
        if (minAge.HasValue && minAge.Value < 0)
            throw new ArgumentException("Minimalny wiek nie może być ujemny", nameof(minAge));

        if (maxAge.HasValue && maxAge.Value < 0)
            throw new ArgumentException("Maksymalny wiek nie może być ujemny", nameof(maxAge));

        if (minAge.HasValue && maxAge.HasValue && minAge.Value > maxAge.Value)
            throw new ArgumentException("Minimalny wiek nie może być większy od maksymalnego");
